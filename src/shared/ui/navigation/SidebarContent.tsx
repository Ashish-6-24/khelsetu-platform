  ...
  return (
    <li>
      <Link
        ...
        className={clsx(
          'group relative flex items-center gap-3 rounded-xl px-2.5 py-2 text-sm font-medium transition-all duration-150 min-h-11',
          active
          ...
          <ul className="space-y-0.5">
            {group.items.map((item) => (
              <NavItem
                ...
                collapsed={collapsed}
                isPending={isPending}
                onClick={() => startTransition(() => onItemClick?.())}
              />
            ))}
          </ul>
        </div>
