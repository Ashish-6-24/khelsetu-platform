    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 text-sm sm:gap-4">
        <div className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2 backdrop-blur-xl dark:bg-gray-800/80">
          ...
          <span className="text-gray-600 dark:text-gray-300">{stats.images} images</span>
        </div>
        <div className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2 backdrop-blur-xl dark:bg-gray-800/80">
          <Video className="h-4 w-4 text-purple-500" />
          <span className="text-gray-600 dark:text-gray-300">{stats.videos} videos</span>
        </div>
        <div className="flex items-center gap-2 rounded-lg bg-white/80 px-3 py-2 backdrop-blur-xl dark:bg-gray-800/80">
          <Trophy className="h-4 w-4 text-[#b8860b]" />
          <span className="text-gray-600 dark:text-gray-300">{stats.highlights} highlights</span>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="aspect-square animate-pulse rounded-xl bg-gray-200 dark:bg-gray-700" />
          ))}
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center py-16">
          <p className="text-lg font-medium text-red-500">Error loading media</p>
