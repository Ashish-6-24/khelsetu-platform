...
function buildBarData(teamA: TeamStatistics, teamB: TeamStatistics) {
  return [
    ...
    { stat: 'On Target', teamA: teamA.shotsOnTarget, teamB: teamB.shotsOnTarget },
    { stat: 'Passes', teamA: teamA.successfulPasses, teamB: teamB.successfulPasses },
...
function buildRadarData(teamA: TeamStatistics, teamB: TeamStatistics) {
  return [
    { subject: 'Attacking', teamA: teamA.shotsOnTarget, teamB: teamB.shotsOnTarget },
    { subject: 'Possession', teamA: teamA.possession, teamB: teamB.possession },
    { subject: 'Passing', teamA: teamA.passAccuracy, teamB: teamB.passAccuracy },
    { subject: 'Defense', teamA: teamA.saves, teamB: teamB.saves },
    { subject: 'Discipline', teamA: 10 - teamA.yellowCards - teamA.redCards, teamB: 10 - teamB.yellowCards - teamB.redCards },
...
export const StatChart = ({
  ...
  if (type === 'bar') {
    ...
    return (
      <div className="rounded-2xl border border-gray-200 bg-white/80 p-5 backdrop-blur-xl dark:border-gray-700 dark:bg-gray-800/80">
        ...
        <div role="img" aria-label={`${title} chart comparing ${teamAName} and ${teamBName}`}>
          <ResponsiveContainer width="100%" height={300}>
            ...
            <Tooltip
              content={<CustomTooltip teamAName={teamAName} teamBName={teamBName} />}
            />
            <Legend
              wrapperStyle={{ fontSize: 12, paddingTop: 8 }}
              formatter={(value: string) => (value === 'teamA' ? teamAName : teamBName)}
            />
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white/80 p-5 backdrop-blur-xl dark:border-gray-700 dark:bg-gray-800/80">
      ...
      <div role="img" aria-label={`${title} radar chart comparing ${teamAName} and ${teamBName}`}>
        <ResponsiveContainer width="100%" height={300}>
